...
function updateprod(){
  ...
  game.production.etime=game.production.energy*(Math.max(Math.pow(Math.floor(game.currency.existance),0.05)-1.4,0)+Math.max(Math.pow(game.currency.existivity,0.001)-1.0075,0));
}
function updatecurr(){
  if (lasttime!==0){
    d=new Date();
